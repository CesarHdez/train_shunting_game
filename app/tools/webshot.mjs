...
if (process.env.SKIP === '1') {
  // Level 1 shows the guided tutorial; levels with a second locomotive show a
  // one-time "¡DOS LOCOMOTORAS!" discovery hint. Dismiss whichever is present
  // (each call just times out and returns false when its overlay isn't there).
  await tapText(/Saltar tutorial/, 2500).catch(() => {});
  await tapText(/ENTENDIDO/, 2500).catch(() => {});
  await wait(900);
}
await wait(1200);
const CLIP = process.env.CLIP ? JSON.parse(process.env.CLIP) : undefined;
